...
class OptimizationScript():
    ...
    def get_ground_truth(self):
        ...
        power3 = []
        power2 = []
        for file in self.files[:1000]:
            beta_cloud = np.load(file)['lwc']
            beta_cloud = beta_cloud.astype(float)
            FS3 = np.fft.fftshift(np.abs(np.fft.fftn(beta_cloud)**2))
            power3.append(FS3)
            FS2 = np.fft.fftshift(np.abs(np.fft.fft2(beta_cloud,axes=(0,1))**2),axes=(0,1))
            power2.append(FS2)
        savemat('cloud_PS.mat',{'power3':power3,'power2':power2})
